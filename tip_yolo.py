...
class YOLO3DefaultTrainTransform_fake(object):
...
class YOLO3DefaultValTransform_fake(object):
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        ...
        train_dataset = gcv.data.RecordFileDetection('./ssd/yolo_data/train.rec')
        #val_dataset = gdata.VOCDetection(
        #    splits=[(2007, 'test')])
        val_dataset = gcv.data.RecordFileDetection('./ssd/yolo_data/val.rec')
        val_metric = VOC07MApMetric(iou_thresh=0.5, class_names=['traffic light', 'auxiliary left light', 'auxiliary right light', 'train light'])
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, args):
    ...
    if args.no_random_shape:
        train_loader = gluon.data.DataLoader(
            train_dataset.transform(YOLO3DefaultTrainTransform_fake(width, height, net, mixup=args.mixup)),
            batch_size, True, batchify_fn=batchify_fn, last_batch='rollover', num_workers=num_workers)
    else:
        transform_fns = [YOLO3DefaultTrainTransform_fake(x * 32, x * 32, net, mixup=args.mixup) for x in range(10, 20)]
        train_loader = RandomTransformDataLoader(
            transform_fns, train_dataset, batch_size=batch_size, interval=10, last_batch='rollover',
            shuffle=True, batchify_fn=batchify_fn, num_workers=num_workers)
    val_batchify_fn = Tuple(Stack(), Pad(pad_val=-1))
    val_loader = gluon.data.DataLoader(
        val_dataset.transform(YOLO3DefaultValTransform_fake(width, height)),
...
if __name__ == '__main__':
    ...
    if args.syncbn and len(ctx) > 1:
        ...
        print('come here')
        net = get_model(net_name, classes=classes, pretrained_base=True, transfer=args.dataset)
        net.load_parameters('yolo3_darknet53_custom_0000_0.9075.params')
        async_net = net
