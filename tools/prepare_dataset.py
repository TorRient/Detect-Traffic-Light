...
from dataset.trafficlight import TrafficLight
...
def load_TrafficLight(image_set, year, devkit_path, shuffle=False, class_names=None, true_negative=None):
    image_set = [y.strip() for y in image_set.split(',')]
    assert image_set, "No image_set specified"
    year = [y.strip() for y in year.split(',')]
    assert year, "No year specified"
    ...
    imdbs = []
    for s, y in zip(image_set, year):
        imdbs.append(TrafficLight(s, y, devkit_path, shuffle, is_train=True, class_names=class_names, true_negative_images=true_negative))
    if len(imdbs) > 1:
        return ConcatDB(imdbs, shuffle)
    else:
        return imdbs[0]
...
if __name__ == '__main__':
    ...
    if args.dataset == 'pascal':
        ...
        print("saving list to disk...")
        db.save_imglist(args.target, root=args.root_path)
    elif args.dataset == 'traffic_light':
        db = load_TrafficLight(args.set, args.year, args.root_path, args.shuffle, args.class_names, args.true_negative)
        print("saving list to disk...")
        db.save_imglist(args.target, root=args.root_path)
    else:
    ...
    im2rec_path = os.path.join(os.getcwd(), 'mxnet/tools/im2rec.py')
    subprocess.check_call(
        ["python", im2rec_path, os.path.abspath(args.target), os.path.abspath(args.root_path), "--shuffle",
         str(int(args.shuffle)), "--pack-label", "1"])
